...
def plot_fft(CI_values, pos, max_pos, legend):
    ...
    plt.plot(
        f[: N // 2], np.abs(fft)[: N // 2], "+"
    )  # Only plot the frequencies in the positive half of the spectrum
...
def plot_raw_data(ax, countries, df_dict, start_date, end_date):
    ...
    for country_idx, country in enumerate(countries):
        df_to_plot = df_dict[country][
            df_dict[country]["datetime"].between(start_date, end_date)
        ]
        plt.plot(
...
def plot_heatmap(ax, similarity_matrix):
    ...
    plt.subplots_adjust(
        left=0.325
    )  # adjust the position of the heatmap: increase the left margin
...
def display_CI(start_date, end_date, countries, df_dict):
    ...
    for key in countries:
        ...
        df_country[df_country["datetime"].between(start_date, end_date)].plot(
            y="CI_direct", x="datetime", ax=ax, figsize=(7, 3), label=key, grid=True
        )
